...
포인트에서 findAndModify를 안 쓰고 MySQL로 간 이유는, 포인트는 **차감 + 이력 저장**이 반드시 같이 이루어져야 하기 때문이다. findAndModify는 단일 문서 연산이라 "포인트 차감"과 "이력 저장"을 하나로 묶을 수 없다. 차감은 됐는데 이력이 누락되는 구조적 위험이 남는다. MySQL의 `@Transactional`은 여러 테이블에 걸친 연산을 하나의 트랜잭션으로 묶을 수 있으므로, 차감과 이력이 반드시 같이 성공하거나 같이 실패한다.

| 도메인 | 연산 | 해결책 | 이유 |
|--------|------|--------|------|
| 재고 | 숫자 1개 차감 | MongoDB `findAndModify` | 단일 문서 원자성으로 충분 |
| 포인트 | 잔액 차감 + 이력 저장 | MySQL `@Transactional` + `SELECT FOR UPDATE` | 다중 테이블 트랜잭션 필수 |

#### 성능 특성의 차이

기능적으로는 둘 다 동시성을 보장하지만, 대규모 동시 요청에서 성능 차이가 크다.

**MongoDB `findAndModify`** 는 DB 엔진 내부에서 조건 체크 + 수정을 하나의 명령으로 처리한다. 행을 잠그고 대기하는 구조가 아니라, 명령 자체가 원자적이므로 잠금 시간이 극히 짧다. 10,000명이 동시에 요청해도 순차 처리가 빠르게 끝난다.

**MySQL `SELECT FOR UPDATE`** 는 행을 잠그고 트랜잭션이 끝날 때까지 잠금을 유지한다. 잠금 시간 동안 같은 행에 접근하는 다른 트랜잭션은 전부 대기한다. 트랜잭션 안에서 여러 테이블을 조작할 수 있는 대신, 동시 요청이 많으면 대기 큐가 쌓이고 커넥션 풀 부하가 올라간다.

| | MongoDB `findAndModify` | MySQL `SELECT FOR UPDATE` |
|---|---|---|
| 정합성 | 보장 | 보장 |
| 원자성 범위 | 단일 문서 | 다중 테이블 (트랜잭션) |
| 잠금 방식 | 잠금 없음 (명령 자체가 원자적) | 행 잠금 (트랜잭션 종료까지) |
| 동시 10,000명 | 빠름 (대기 없음) | 느림 (잠금 대기 큐) |
| 커넥션 풀 부하 | 낮음 | 높음 (대기 중 커넥션 점유) |

k6 재고 동시성 테스트에서 이 차이가 드러났다. 300명 → 3,000명 → 10,000명으로 동시 접속자를 늘려도 `findAndModify`는 정확히 재고(100건)만 성공시키고 나머지를 거절했다. 요청 수에 관계없이 성공 건수가 재고로 고정되는 이유는 애플리케이션 레벨 잠금이 아니라 DB 엔진 레벨 원자성이기 때문이다. 로컬 하드웨어의 한계로 10,000명 이상은 테스트하지 못했지만, 이 선형적 패턴이 유지되므로 더 큰 규모에서도 정합성은 보장될 것으로 판단한다.

이 과정에서 Polyglot Persistence의 의미를 체감했다. 같은 "숫자를 빼는" 연산이라도, 도메인 요구사항에 따라 적합한 DB와 해결책이 다르다는 것을 배웠다. 재고처럼 단일 값 차감만 필요한 도메인은 MongoDB가 성능상 유리하고, 포인트처럼 차감 + 이력을 묶어야 하는 도메인은 MySQL이 필요했다.
