...
2. 역순 롤백: 카드 → 포인트 순서로 보상 (실행 역순)
3. 보상 실패도 처리: 포인트 복구 자체가 실패할 수 있으므로 try-catch로 감싸고 로그 남김
...
SAGA도 마찬가지다. "분산 트랜잭션에는 보상 트랜잭션이 필요하다"는 건 책에서 읽었지만, 실제로 포인트가 날아가는 걸 부하 테스트 수치로 확인하고, 보상 코드를 짜고, 다시 테스트해서 99.2%가 복구되는 걸 보기 전까지는 왜 필요한지 와닿지 않았다.
