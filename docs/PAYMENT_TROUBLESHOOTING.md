...
"재고 부족이면 거절" 로직이 있어도, **조회와 차감 사이에 잠금이 없으면 300명이 전부 `stock=100`을 읽고 전부 통과한다.** 이것이 Read-Check-Write 패턴의 근본적 문제다.
...
이것이 Polyglot Persistence의 실제 사례다. 같은 "숫자를 빼는" 연산이라도, 도메인 요구사항에 따라 적합한 DB와 해결책이 다르다.
