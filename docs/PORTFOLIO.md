...
| 항목 | 수치 |
|------|------|
| SAGA 보상 트랜잭션 | 포인트 유실률 90.4% → 0% (SAGA + Outbox, 장애 주입 10%) |
| 동시성 제어 | 마이너스 잔액 0건, 재고 초과 판매 0건 (findAndModify + SELECT FOR UPDATE) |
| 부하 테스트 | 동시 300명 + 장애 주입 10%, Before/After 정량 비교 |
