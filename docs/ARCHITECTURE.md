...
### 구현 완료
- [x] SAGA 패턴 (Orchestration) — 결제 시 포인트 차감 → 카드 결제 → DB 저장, 실패 시 역순 보상(카드 환불 → 포인트 복구 → 재고 복구 → 적립 포인트 회수) + CompensationOutbox 재시도. 결제 3초 타임아웃 + 부하로 자연 실패를 유도해 검증 (포인트 유실률 90.4% → 0%)
- [x] 동시성 제어 — point-service: MySQL `SELECT FOR UPDATE` 비관적 락 + Redis 분산 락(Redisson). 단일 인스턴스에서는 DB 락이 처리량 68% 우위, 3대 스케일아웃 시 Redis 락이 처리량 82% 증가·p95 55% 개선으로 역전 ([부하 테스트 결과](LOAD_TEST.md))
