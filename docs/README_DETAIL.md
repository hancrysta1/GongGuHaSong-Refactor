...
| 아키텍처 | 모놀리식 Spring Boot | MSA 6개 서비스 + 인프라 6개 → Kubernetes 전환 |
...
| Docker Compose | 로컬 개발 | 6개 서비스 + 인프라 통합 배포 |
| Kubernetes | 운영/스케일 아웃 | 3대 Pod 분배 검증, self-healing, Rolling Update |
...
### 다중 인스턴스 & Kubernetes — [MULTI_INSTANCE.md](MULTI_INSTANCE.md)

- Docker Compose vs K8s 개념 비교, Pod/Deployment/Service/ClusterIP/kube-proxy 용어 정리
- kube-proxy 분배 원리 (iptables, TCP 연결 단위), Eureka Ribbon과의 차이
- 자원 비교: 앱 레벨(Eureka) vs 인프라 레벨(kube-proxy) — 서비스 10개 기준 ~1.4GB vs 거의 0
- Netflix/배민/카카오/토스/쿠팡/라인 기업 전환 사례
- AI 시대 개발자가 인지해야 할 것: 설정은 AI가 해주지만 "왜 안 되지?" 판단력은 삽질로만 생긴다.

### Kubernetes 전환 트러블슈팅 — [TROUBLESHOOTING3.md](TROUBLESHOOTING3.md)

- Kafka CrashLoopBackOff: K8s Service 환경변수 자동 주입(`KAFKA_PORT`) → Confluent 이미지 충돌 → `enableServiceLinks: false`
- port-forward 1대 고정: `kubectl port-forward`는 디버깅용 터널, 로드밸런서 아님 → 서비스 간 호출로 전환
- ES Nori 플러그인 누락: K8s 공식 이미지에 Nori 미포함 → 커스텀 이미지 필요
- HTTP Keep-Alive 분배 안 됨: Java RestTemplate TCP 재사용 → kube-proxy 고착 → 1차(`@LoadBalanced` 제거 실패) → 2차(원인: TCP 연결 단위) → 3차(`Connection: close` 해결)
...
| K8s 스케일 아웃 | point-service 3대 분배 확인, 잔액 정확(8,000P), 마이너스 0건 |
| 인프라 | Docker Compose → Kubernetes 전환 |
...
### Kubernetes
```bash
# Docker Desktop K8s 활성화 후
kubectl apply -f k8s/infra.yml
kubectl apply -f k8s/services.yml

# 스케일 아웃 테스트
kubectl scale deployment point-service --replicas=3
kubectl logs -l app=point-service -f --prefix   # 분배 확인
```

---

## Kubernetes 스케일 아웃 — [MULTI_INSTANCE.md](MULTI_INSTANCE.md)

Docker Compose → Kubernetes로 전환하여 서비스 스케일 아웃을 검증했다.

| | Docker Compose | Kubernetes |
|---|---|---|
| 스케일링 | yml 수동 정의, 포트 변경 | `kubectl scale --replicas=3` 한 줄 |
| 로드밸런싱 | Eureka + Ribbon (앱 레벨, JVM마다 라이브러리) | kube-proxy + iptables (커널 레벨, 앱 무관) |
| 자동 스케일링 | 불가능 | HPA (CPU 기반 자동 확장) |
| 장애 복구 | 수동 재시작 | Pod 자동 재생성 (self-healing) |
| 무중단 배포 | 서비스 중단 | Rolling Update |
| 자원 비용 (10서비스×3대) | Eureka Server ~512MB + Client ~900MB | kube-proxy iptables 규칙 수 KB |

검증 결과.
- point-service 3대 Pod에 결제 20건이 분배되어 처리됨 (Pod1: 5건, Pod2: 9건, Pod3: 6건)
- 잔액 정확히 8,000P (10,000 - 20×100), 마이너스 잔액 0건
- MySQL `SELECT FOR UPDATE`가 3대 인스턴스에서도 정상 동작 (DB 레벨 락이라 인스턴스 수 무관)

K8s 전환 과정에서 4건의 트러블슈팅 (Kafka 환경변수 충돌, port-forward 고착, HTTP Keep-Alive 분배, ES Nori 누락)을 경험하고 해결했다. ([TROUBLESHOOTING3.md](TROUBLESHOOTING3.md))

Eureka vs kube-proxy 자원 비교, Netflix/배민/카카오/토스 등 기업 전환 사례, kube-proxy 동작 원리, AI 시대 개발자가 인지해야 할 점도 정리했다. ([MULTI_INSTANCE.md](MULTI_INSTANCE.md))

---

## 향후 개선 방향

- HPA 자동 스케일링: 현재 수동 `kubectl scale` → CPU 기반 자동 확장 (클라우드 환경 필요)
- Eureka 제거: 순수 K8s 환경에서는 K8s Service DNS만으로 충분. 현재는 `@EnableEurekaClient` 호환성 때문에 남김
- Outbox Pattern: SAGA 보상 실패 건(현재 0.08%) 재처리
- Circuit Breaker: 서비스 장애 시 빠른 실패 (Resilience4j)
- 분산 추적: Zipkin/Jaeger로 서비스 간 요청 추적
- 클라우드 K8s (GKE): 멀티 노드 + HPA 자동 스케일링 실측
...
├── k8s/                  # Kubernetes 매니페스트 (infra.yml, services.yml)
