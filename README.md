...
| 대용량 부하 시 DB 락 병목 | p95 469ms, 커넥션 점유 대기로 처리 한계 | Redis 분산 락 적용 → 3대 환경에서 처리량 34%↑, p95 35%↓ |
...
=> timeout만으로 커넥션 풀 병목을 단정할 수는 없으나, <br>

DB 락 API에서만 집중 발생한 점에서 커넥션 점유 대기가 의심되었다. <br>
...
Redis 분산 락은 Redis에서 먼저 락을 획득한 후에 `@Transactional`이 시작되기 때문에 커넥션을 미리 점유하지 않는다.
<br>
역시나 금액 정합성은 둘 다 보장되었다.
<br>
<br>
<br>
...
```
50→300 VU 단계별 부하 + 장애 주입 10% (포인트 차감 후 결제 저장 전 강제 예외)
...
- DB 비관적 락의 구조적 한계 확인 → Redis 분산 락(Redisson) 적용. `RedisLockPointFacade`에서 Redis 락 획득 후 별도 Bean(`RedisLockPointInnerService`)의 `@Transactional`을 호출하여, 락 대기를 트랜잭션 바깥으로 분리 → DB 커넥션 점유 없이 동시성 제어
