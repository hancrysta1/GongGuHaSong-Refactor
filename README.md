...
=> 정합성은 완벽하고, 성공률 50%는 적립/차감 반반 설계에서 잔액 부족 거절이 포함된 정상 수치다. <br>
=> 다만 **p95 469ms, max 2.26s**로 300 VU에서 지연이 쌓이기 시작했다. <br>
=> SELECT FOR UPDATE로 같은 행을 잡으려는 커넥션들이 대기하면서 지연이 발생하는 구조이며, 트래픽이 더 늘면 대기 시간이 기하급수적으로 증가한다. <br>
=> 인스턴스를 늘려 처리량을 확보하려 해도, DB 비관적 락은 커넥션 풀이 늘어난 만큼 같은 행에 대한 경합이 심화되어 오히려 병목이 된다. <br>
=> 이 한계를 해결하기 위해 Redis 분산 락을 적용하여, 락 대기를 DB 밖으로 분리하고 멀티 인스턴스 환경에서의 성능을 비교했다.

<br>
<br>
<br>

### 부하 테스트(2-1) Redis 분산 락 <br>

```
단일 인스턴스에서는 DB SELECT FOR UPDATE가 빠르지만,
대용량 트래픽을 처리하기 위해 인스턴스를 늘리면
여러 커넥션 풀이 동시에 같은 행을 잠그면서 DB 커넥션을 점유한 채 대기한다.
트래픽이 더 몰리면 커넥션 풀이 고갈되고, 락과 무관한 조회까지 영향을 받는다.

Redis 분산 락은 락 대기를 DB 밖(Redis)에서 처리하여
DB 커넥션을 실제 작업할 때만 사용하므로 커넥션 풀 고갈이 발생하지 않는다.

이 차이를 검증하기 위해 point-service 3대 + nginx 로드밸런서 구성으로
2분간 50→300 VU ramp-up, 총 4만 건 이상의 부하를 걸어 비교했다.
```
...
|------|------------|-------------|
| 처리량 (2분) | 30,145건 | **40,580건** |
| TPS | 251 req/s | **338 req/s** |
| avg 지연 | 278ms | **292ms** |
| p95 지연 | 1.09s | **704ms** |
| 성공률 | 100% | 100% |
| 마이너스 잔액 | 0명 | 0명 |
...
=> point-service 3대, nginx 로드밸런서, 300 VU 동시 부하, 2분간 약 338 TPS로 총 4만 건 처리 <br>
=> **Redis 분산 락이 처리량 34% 높고, p95 지연 35% 낮음** <br>
=> DB 비관적 락은 커넥션 풀 3개(총 30개)가 같은 행에 FOR UPDATE → 커넥션을 점유한 채 대기 → 풀 고갈로 성능 저하 <br>
=> Redis 분산 락은 락 대기가 Redis에서 일어나 DB 커넥션 점유 없음 → 커넥션 풀 고갈 방지 <br>
=> 정합성은 양쪽 모두 완벽 (마이너스 잔액 0건). 대용량 환경에서는 Redis 분산 락이 적합
...
- 재고 동시성: MongoDB `findAndModify` 원자적 차감 → 300명/3,000명/10,000명 동시 주문에서 초과 판매 0건
- 포인트 동시성: MongoDB `findAndModify` 한계 발견 → MySQL `SELECT FOR UPDATE` 리팩토링 → 21만 건 동시 적립/차감에서 마이너스 잔액 0건
...
- 포인트 동시성: 300 VU × 5분, 21만 건 동시 적립/차감에서 마이너스 잔액 0건
- DB 락 vs Redis 락: 대용량 부하 환경(3대, 300VU)에서 DB 비관적 락의 커넥션 풀 고갈 문제 확인 → Redis 분산 락이 처리량 34% 높고 p95 35% 낮음
